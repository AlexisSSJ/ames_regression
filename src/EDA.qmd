...
(
                ...
                theme_seaborn()+ theme(figure_size=(11, 7))
...
(
    ...
    + scale_fill_gradient2(low='#4EB043',
                           ...
                           name='Correlación')
    + theme_bw()
    + theme(axis_text_x=element_text(angle=90), figure_size=(11, 7))
)
```
...
(
  ames_train 
        ...
        + scale_y_continuous(labels=dollar_format(big_mark=',', digits = 0)) 
        +coord_flip()
        +labs(y='Precio  promedio por pie cuadrado de área habitable', x='')+ theme(figure_size=(11, 7))
)
```
...
(
    ames_x_train 
        ...
        + theme_538()+ theme(figure_size=(11, 7))
)
```
...
for category in cat_columns[cat_columns.str.contains('Bsmt')]:
  ...
  (ames_train  >> ggplot(aes(y= 'Sale_Price/Gr_Liv_Area', x= 'Total_Bsmt_SF', color= '{}'.format(category)))
              ...
              + facet_wrap('~{}'.format(category)) +theme_minimal() +theme(figure_size=(11, 7)))
```
...
results_reg = (
  ames_x_test >>
  mutate(final_lm_pred = pipeline_linreg.predict(ames_x_test)*_.Gr_Liv_Area, 
        Sale_Price = Sale_Price_test, cat = 'test') >>
  select(_.Sale_Price, _.final_lm_pred, _.cat))
  
results_val = (
  ames_x_val >>
  mutate(final_lm_pred = pipeline_linreg.predict(ames_x_val)*_.Gr_Liv_Area , 
        Sale_Price = Sale_Price_validation, cat = 'validation') >>
  select(_.Sale_Price, _.final_lm_pred, _.cat) )
  
(
    pd.concat([results_reg, results_val])
        >> ggplot (aes (x = 'final_lm_pred', y ='Sale_Price', color = 'cat'))
        ...
        + labs(
              ...
              x = "Predicción",
              y = "Observación", color ='Datos')+theme_tufte()
)
...
test >>
  ...
  ggtitle("Distribución de error")+theme_tufte()+ theme(figure_size=(11, 7))
)
...
test >>
  ...
  ggtitle("QQ-Plot") +theme_tufte()+ theme(figure_size=(11, 7))
)
```
...
(
  ...
  coord_flip() +theme_minimal()+ theme(figure_size=(11, 7))
)
```

# Random Forest 

Para encontrar una configuración adecuada para el modelo, se buscaron los mejores hiper-parámetros para el full-model, posteriormente se elmiminaron variables por importancia de estas, hasta obtener una buena métrica. Una vez se seleccionaron las variables se volvió a correr un grid search para optimizar el modelo modificando sus hiper-parámetros.
...
En los resultados de grid search se obtuvo que el mejor modelo de acuerdo a su $R^2_{adj}$ fue el siguiente:
...
(
  ...
  ylab("R^2 promedio") +
  ggtitle("Parametrización de Random Forest vs R^2 - TOP 20 mejores CV scores")+ theme(figure_size=(10, 6))
)
```

```{python}
#| label: preliminar_model fit
#| code-fold: false
#| results: hide
...
A continuación se muestrann los resultados de los modelos con haciendo backward selection tomando como criterio la importancia de la variable, el modelo seleccionado fue el que se queda con 34 predictores, el cuál alcanza una $R^2_{adj} = 0.863$.

```{python}
#| label: importances from preliminar model
#| code-fold: false
...
importances 
    ...
    + coord_flip() + theme(figure_size=(11, 7))
)
...
```{python}
#| label: importance iterations removing
#| eval: false
#| code-fold: false
v=[]
r2=[]
...
(
    results_imp_tunning 
        ...
        + labs(title = 'Resultados diferentes modelos Radom Forest', x='Predictores', y= 'Métrica')
        +  geom_point()
...
selected_vars = ['MS_Zoning_RL', 'Bsmt_Qual_Gd', 'Condition_1_infrequent_sklearn', 'Bsmt_Qual_TA', 'Condition_1_Norm', 'Enclosed_Porch', 'Alley_No_Alley_Access', 'Bsmt_Qual_Gd_x_Bsmt_Prop_c_', 'Central_Air_Y', 'House_Style_1Story', 'Kitchen_Qual_TA', 'Lot_Config_Inside', 'Neighborhood_Crawfor_x_Gr_Liv_Area', 'Full_Bath', 'Garage_Finish_RFn', 'Garage_Cars', 'Overall_Qual_Good_or_Excellent', 'Kitchen_Qual_Gd', 'Neighborhood_NoRidge_x_Gr_Liv_Area', 'Antique_c_', 'Garage_Area', 'Gr_Liv_A/Loot_A_c__x_Gr_Liv_A/Loot_A_c_', 'Year_Built', 'Neighborhood_NridgHt_x_Gr_Liv_Area', 'TotRms_AbvGrd', 'Bedroom_AbvGr', 'Lot_Area', 'Area_Per_Room_c_', '2nd_Flr_SF', 'Fireplaces', 'Exter_Qual_Gd', 'Total_Bsmt_SF', '1st_Flr_SF', 'Gr_Liv_Area']
#(results >> filter (_.P==34) >> pull("used_vars"))[0]
```

## Segunda ronda de CV

Para poder mejorar aúnn más el modelo se hace otro grid search en busca de mejores hiper-parámetros para este:
...
(
  ...
  xlab("Parameter value") +
  ylab("R^2 promedio") +
  ggtitle("Parametrización de Random Forest vs R^2") + theme(figure_size=(11, 7))
)
```

Con los resultados del grid search se decide escoger los siguientes hiper-parámetros para el modelo final:
...
#| code-fold: false
#| results: hide
rf2 = RandomForestRegressor(n_estimators = 70, max_depth=12, max_features=21, min_samples_leaf=4,min_samples_split=5, random_state =7, n_jobs = 6)
...
results_reg = (
  ames_x_test >>
  mutate(
          pred = pipeline_cv_final.predict(ames_x_test)*_.Gr_Liv_Area, 
          Sale_Price = Sale_Price_test, 
          cat = 'Test'
        ) >>
  select(_.Sale_Price, _.pred, _.cat))
  
results_val = (
  ames_x_val >>
  mutate(pred = pipeline_cv_final.predict(ames_x_val)*_.Gr_Liv_Area , 
        Sale_Price = Sale_Price_validation, cat = 'Validation') >>
  select(_.Sale_Price, _.pred, _.cat) )
...
results_rf= pd.DataFrame(
  {'Resultados Test': list(get_metrics(results_reg.pred,results_reg.Sale_Price, len(selected_vars)).Value),
  "Resultados Validación" : list(get_metrics(results_val.pred ,results_val.Sale_Price, len(selected_vars)).Value)}, index = np.array(['ME', 'MAE', 'MAPE', 'MSE', 'RMSE', 'R2', 'R2Adj']))

```

```{python}
#| label: tbl_results_rf2
#| output: asis
#| echo: false
results_rf[['Resultados Test']].to_html()
```

## Segunda selección de variables
Se calcula la importancia de las variables nuevamente y se vuelve a hacer una selección de estas.

```{python}
#| label: importancia rf2
#| code-fold: false
...
y_pred = pipeline_cv_final.predict(ames_x_test) * ames_x_test.Gr_Liv_Area
y_obs = Sale_Price_test
...
Nuevamente se vuelve a iterar para eliminar variables, se eliminan las 7 variables menos importantes y nos quedamos con un modelo de únicamente 27 variables.

```{python}
#| label: 2dn round vars importance rf
#| eval: false
#| code-fold: false
...
```{python}
#| label: variables selection final rf
...
(
    results 
        ...
        + facet_wrap("~metric",ncol = 1, scales  = 'free_y')
        + labs (
            title = 'Resultados haciendo Backward selection',
            x = 'Predictores usados',
            y = 'Métrica')
)
```
## Resultados en test y validación
Finalmente se escoge el siguiente modelo que alcanza una $R^2_{adj}\approx 0.888$ en test:
```{python}
#| label: final pipe rf and results
#| code-fold: false
#| results: hide
# (results >> top_n(1, _.R_adj) >> pull("used_vars"))[0]
best_estimator = RandomForestRegressor(
                                n_estimators = 150, 
                                max_depth=12, 
                                max_features=19, 
                                ...
                                random_state =7, 
                                n_jobs = 6)
selected_vars2 = ['Enclosed_Porch', 'Exter_Qual_Gd', 'Full_Bath', 'Neighborhood_Crawfor_x_Gr_Liv_Area', 'Kitchen_Qual_Gd', 'Garage_Cars', 'Overall_Qual_Good_or_Excellent', 'Bsmt_Qual_Gd', 'Fireplaces', 'MS_Zoning_RL', 'Kitchen_Qual_TA', 'House_Style_1Story', 'Neighborhood_NridgHt_x_Gr_Liv_Area', 'Central_Air_Y', 'Bedroom_AbvGr', 'Lot_Area', 'Area_Per_Room_c_', 'Garage_Area', '1st_Flr_SF', 'Gr_Liv_A/Loot_A_c__x_Gr_Liv_A/Loot_A_c_', 'TotRms_AbvGrd', 'Gr_Liv_Area', 'Total_Bsmt_SF', 'Bsmt_Qual_Gd_x_Bsmt_Prop_c_', '2nd_Flr_SF','Year_Built', 'Antique_c_']
...
results_reg = (
  ames_x_test >>
  mutate(
          pred = rf_pipline.predict(ames_x_test)*_.Gr_Liv_Area, 
          Sale_Price = Sale_Price_test, 
          cat = 'Test'
        ) >>
  select(_.Sale_Price, _.pred, _.cat))
...
results_val = (
  ames_x_val >>
  mutate(pred = rf_pipline.predict(ames_x_val)*_.Gr_Liv_Area , 
        Sale_Price = Sale_Price_validation, cat = 'Validation') >>
  select(_.Sale_Price, _.pred, _.cat) )
  
(
   pd.concat([results_reg, results_val])
        >> ggplot (aes (x = 'pred', y ='Sale_Price', color = 'cat'))
        +  geom_point(alpha = 0.6)
...
results_rf = pd.DataFrame(
  {'Resultados Test': list(get_metrics(results_reg.pred,results_reg.Sale_Price, len(selected_vars2)).Value),
  "Resultados Validación" : list(get_metrics(results_val.pred ,results_val.Sale_Price, len(selected_vars2)).Value)}, index = np.array(['ME', 'MAE', 'MAPE', 'MSE', 'RMSE', 'R2', 'R2Adj']))
```

```{python}
#| label: tbl_results_rf final
#| output: asis
#| echo: false
results_rf.to_html()
```
